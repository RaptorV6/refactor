...
import type { CdrCalendarEvent } from "./cdr-calendar-loaders";
...
export const CdrCalendarEventComponent = component$<CdrCalendarEventComponentProps>(({ calendar, dates, event }) => {
  ...
  const dateTo = event.dateTo ?? event.dateTo ?? dateAddMinutes(event.duration ?? 30, dateFrom);
  ...
  const dateIndex = dates.findIndex((d) => datesEq(d.date, dateFrom));
    ...
    >
      ...
      >
        ...
        <div class="order-2 flex items-center justify-between text-left text-[0.6rem]/5 text-blue-500 group-hover:text-blue-700">
          <div>
            {event.medicalProcedure.roomIsShared ? (
              <svg
